...
class UserRequestController_Legacy extends BaseController
{
...
//        Log::info('Processing requests', ['count' => $requests->count(), 'type' => $type, 'requests' => $requests]);
        foreach ($requests as $request) {
            // Use new overall_status column for filtering
            ...
            if ($relevantResponses->isNotEmpty()) {
                ...
                foreach ($relevantResponses as $response) {
                    ...
                    $requestCopy->response_status = $response->overall_status;
                    $requestCopy->response_type = $response->response_type;
                    $requestCopy->user_role = $response->getUserRole($currentUser->id);
                    $requestCopy->user_status = $response->getUserStatus($currentUser->id);
                    ...
                    // Use new overall_status for determining request status
                    if (in_array($request->status, ['closed', 'completed'])) {
                        $requestCopy->status = $request->status; // Keep original status
                    } else {
                        $overallStatus = $response->overall_status;
                        if ($overallStatus === 'accepted') {
                            // Both users have accepted - show matched status
                            $requestCopy->status = 'matched';
                        } else if ($overallStatus === 'partial') {
                            // One user has accepted, waiting for the other
                            $requestCopy->status = 'has_responses';
                        } else if ($overallStatus === 'pending') {
                            // No one has accepted yet
                            $requestCopy->status = 'has_responses';
                        } else if ($overallStatus === 'closed') {
                            $requestCopy->status = 'closed';
                        }
                    }

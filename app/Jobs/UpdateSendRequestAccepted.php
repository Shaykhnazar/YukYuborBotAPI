...
class UpdateSendRequestAccepted implements ShouldQueue
{
    ...
    public function handle(GoogleSheetsService $googleSheetsService): void
    {
        try {
            ...
            if (!$sendRequest) {
                Log::warning('SendRequest not found for Google Sheets accepted update', [
            ...
            $googleSheetsService->updateRequestResponseAccepted('send', $sendRequest->id, $this->responseReceivedTime);
            ...
            Log::error('Failed to update SendRequest as accepted in Google Sheets', [
                'send_request_id' => $this->sendRequestId,
                'error' => $e->getMessage(),
            ]);
